...
?>
			...
			<?php
				...
				if($usuario){
					...
					if ($sql = $conexao->prepare("SELECT nome, dtNasc, email, telFixo, telCel, end, cid, est, cep, senha, escolas_idEscolas, grupos_idGrupos, anos_idAno FROM alunos WHERE usuario = ?")) {
						$sql->bind_param('i', $usuario);
						$sql->execute();
						$sql->bind_result($nome, $dtNasc, $email, $telFixo, $telCel, $end, $cid, $est, $cep, $senha, $escolas_idEscolas, $grupos_idGrupos, $anos_idAno);
						$sql->fetch();
						if ($nome == ''){
				...
				if( isset( $_POST['editar'] ) ):
					...
					if ($usuario=='' || $nome=='' || $dtNasc=='' || $email=='' || $telFixo=='' ||  $telCel=='' || $end=='' || $cid=='' || $est=='' || $cep=='' || $senha=='' || $escolas_idEscolas=='' || $grupos_idGrupos=='' || $anos_idAno=='') {
						...
						$param = $conexao->prepare("UPDATE alunos SET nome = ?, dtNasc = ?, email = ?, telFixo = ?, telCel = ?, end = ?, cid = ?, est = ?, cep = ?, senha = ?, escolas_idEscolas = ?, grupos_idGrupos = ?, anos_idAno = ? WHERE usuario = ?");
						$param->bind_param('sisiisssssiiis', $nome, $dtNasc, $email, $telFixo, $telCel, $end, $cid, $est, $cep, $senha, $escolas_idEscolas, $grupos_idGrupos, $anos_idAno, $usuario);
						if ($param->execute()) {
			...
			<form action="" method="POST" id="editaaluno">
				...
				<label for="escolas_idEscolas"> Id da Escola: </label>
					<input type="text" placeholder="id de escola" class="form-control" name="escolas_idEscolas" value=<?php echo "'". $escolas_idEscolas . "'"; ?>/>
